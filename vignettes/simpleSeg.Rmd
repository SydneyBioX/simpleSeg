...
data(pancreasImages)
```
Minimum input is the image/image list and the nuclei marker to be used. By default, simpleSeg identifies nuclei marker expression using principal component analysis and individual nuclei via watershedding. Nuclei are dilated out by 3 pixels to capture the cytoplasm.
```{r}
masks <- simpleSeg::simpleSeg(pancreasImages, transform = c("sqrt"), watershed = "distance")

cells = rgbImage(green=0.05 * pancreasImages[[1]][,,1], blue=0.03 * pancreasImages[[1]][,,2], red = 0.05 * pancreasImages[[1]][,,5])
...
```{r}
masks <- simpleSeg::simpleSeg(pancreasImages, nucleus = "H3", transform = "asinh")
...
```{r}
masks <- simpleSeg::simpleSeg(pancreasImages,  nucleus = "H3",transform = "asinh",watershed = "combine")
segmented = paintObjects(masks[[1]], cells, col='#ffffff')
display(segmented)
```
...
```{r}
masks <- simpleSeg::simpleSeg(pancreasImages, nucleus = "H3",transform = "asinh",cellBody = "dilate")
segmented = paintObjects(masks[[1]], cells, col='#ffffff')
display(segmented)
masks <- simpleSeg::simpleSeg(pancreasImages, nucleus = "H3",transform = "asinh",cellBody = "discModel")
segmented = paintObjects(masks[[1]], cells, col='#ffffff')
display(segmented)
masks <- simpleSeg::simpleSeg(pancreasImages, nucleus = "H3",transform = "asinh",cellBody = "CD8a")
segmented = paintObjects(masks[[1]], cells, col='#ffffff')
display(segmented)
```
...
```{r}
masks <- simpleSeg::simpleSeg(pancreasImages, cores = 5)
```
